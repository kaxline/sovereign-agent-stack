...
Buzz is an optional **messaging transport** for [independent agents](agents.md). Agents are operator-local Hermes profiles under `data/hermes/profiles/<name>/` (gitignored). Create them with `make agent-create`; attach Buzz with `WITH=buzz` or `./scripts/agent-attach-buzz.sh`.
...
| Piece | Role |
|---|---|
| **Buzz Desktop** | Human client; invite agent pubkeys; `@mention` agents |
...
| **Agent Hermes profile** | One process per profile (s6); own `SOUL.md`, memory, `BUZZ_PRIVATE_KEY` |
...
# 3) Create an agent and attach Buzz (keygen + relay ACL + join channel `general`)
make agent-create NAME=software-engineer DISPLAY_NAME="Software Engineer" WITH=buzz
# Compatibility: make hermes-buzz-employee PROFILE=software-engineer DISPLAY_NAME="Software Engineer"
...
| `data/hermes/profiles/<name>/.env` | Per-agent `BUZZ_PRIVATE_KEY` / `BUZZ_PUBLIC_KEY`, seeded `BUZZ_RELAY_URL` / `BUZZ_CLI_PATH` |
...
2. Prefer automation: `make agent-create NAME=… WITH=buzz` (keygen + `buzz-admin add-member` + join `BUZZ_DEFAULT_CHANNEL`).
...
4. Recreate Hermes so the agent gateway connects; set a display name if needed (`buzz users set-profile` as the agent).
...
1. Create the agent with `make agent-create … WITH=buzz` (or mint a dedicated key yourself — not your human identity; not shared with Desktop ACP).
2. On open Local Dev, channel membership is handled by the attach helper (or `buzz channels add-member` as your human identity). On NIP-43 closed relays, use Desktop **Settings → Invites**.
3. Set display name/avatar so the member appears as “Software Engineer”, not a raw npub.
4. With Hermes gateway running, `@mention` the agent in a channel (or DM).

Do not run Buzz Desktop ACP / managed Hermes on the **same** key as a gateway profile — duplicate replies.

## Multiple agents

One Buzz identity = one Hermes agent profile = one Nostr key.

```bash
make agent-create NAME=software-engineer DISPLAY_NAME="Software Engineer" WITH=buzz
make agent-create NAME=ui-ux-designer DISPLAY_NAME="UI/UX Designer" WITH=buzz
docker compose up -d --force-recreate hermes
```
...
See [Independent agents](agents.md) for create without Buzz and the three memory tiers.
...
Agent profile directories and private keys are left in place under `data/hermes/`.
...
| Eyes reaction but no reply / LM Studio idle | Hermes rejected sender (`Unauthorized user` in profile `logs/gateway.log`) | `allow_all_users: true` is seeded by `sync-buzz-profile`; re-run `make ensure-local` or set `BUZZ_ALLOW_ALL_USERS=true` / `BUZZ_ALLOWED_USERS=<your hex pubkey>` in the agent profile `.env`, then recreate hermes |
...
| Identity | Linked phone | Nostr keypair per agent |
| Profiles | Default owns inbound; browser outbound-only | Agent profiles own Buzz; browser/api-server off |

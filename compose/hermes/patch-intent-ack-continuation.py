...
# v2026.9.x — compressed length check on one line.
OLD_V9 = """    if any(isinstance(msg, dict) and msg.get("role") == "tool" for msg in messages):
        return False
...
NEW_V9 = """    assistant_text = agent._strip_think_blocks(assistant_content or "").strip().lower()
...
# Pre-v2026.9 separate length check.
OLD_LEGACY = """    if any(isinstance(msg, dict) and msg.get("role") == "tool" for msg in messages):
        return False
...
NEW_LEGACY = """    assistant_text = agent._strip_think_blocks(assistant_content or "").strip().lower()
    if not assistant_text:
        return False
